...
export const fetchEvents = createAsyncThunk(
  "events/list",
  async () => (await get<{ events: Event[] }>("/events")).events,
);
export const createEvent = createAsyncThunk(
  "events/create",
  async (input: Omit<Event, "id" | "status">) =>
    (await post<{ event: Event }>("/events", input)).event,
);
export const submitEvent = createAsyncThunk(
  "events/submit",
  async (id: string) => (await post<{ event: Event }>(`/events/${id}/submit`)).event,
);
export const approveEvent = createAsyncThunk(
  "events/approve",
  async (id: string) => (await post<{ event: Event }>(`/events/${id}/approve`)).event,
);
const slice = createSlice({
  ...
  extraReducers: (b) => {
    b.addCase(fetchEvents.pending, (s) => {
      ...
      .addCase(submitEvent.fulfilled, (s, a) => {
